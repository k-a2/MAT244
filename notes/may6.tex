...
\end{example}
    ...
    Consider the following differential equation:
    \[ \frac{dy}{dt} = ay \]
    where $a \in \RR$. Plot direction fields for $a = -1, 0, 1$.
\end{example}
   \begin{solution}
   When $a = -1$, we have that $\frac{dy}{dt} = -y$, i.e. the slope of the equation is equal to $-y$. Observe that $\frac{dy}{dt}$ is independent of $t$ and therefore will not change with respect to $t$. The direction field is as follows:
